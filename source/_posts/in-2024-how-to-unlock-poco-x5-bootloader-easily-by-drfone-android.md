...
As one of the initial measures, backup the Poco X5 device completely as bootloader unlocking process will wipe all the data off. So, backup all the data like photos, contacts, multimedia files, documents, etc.
...
After selecting the Poco X5 device, click on “Begin Unlock Bootloader”, and then confirm all the dialogue boxes which come your way on screen.

Step 4: Now, you will be presented with four steps to put the Poco X5 device in bootloader mode. Disconnect the Poco X5  device from the PC and turn the Poco X5 device off completely. Press the volume down button along with the power button to switch the Poco X5 device on in bootloader mode.

![htc unlock bootloader](https://images.wondershare.com/drfone/article/2016/12/14822589729198.jpg)

Step 5: Use the volume keys of the Poco X5 device to select Fastboot option along with pressing power button to confirm, after the Poco X5 device is in bootloader mode. Now, connect the Poco X5 device to the computer using a USB cable.
...
Step 7: In the command prompt window, type “fastboot devices” and press enter. Poco X5  will show up in the command prompt.

Note: The drivers have to be installed correctly to see the Poco X5 device in the command prompt. So, if the Poco X5 device does not show up, reinstall HTC Sync Manager and try again after restarting the computer.

Step 8: On HTC Dev’s website third page, click on “proceed to Step 9”. Follow the steps listed and then click on submit. The unlock token code for the Poco X5 device will be mailed by HTC. Download the token and name it “Unlock\_code.bin” and place the token in the fastboot folder.

Step 9: Now, in the command prompt window, type the following:

fastboot flash unlocktoken Unlock\_code.bin

Step 10: On the Poco X5 , one message will appear asking if you want to unlock the Poco X5 device bootloader.

![htc unlock bootloader](https://images.wondershare.com/drfone/article/2016/12/14822585759420.jpg)

Use volume keys to select and power button to confirm. Once this is done, the Poco X5  device will restart once and it’s done. The device is now bootloader unlocked.
